...
class UserApi {
  ...
  async getUserByOrgWithGranted({customerId,botId,pagination}) {
    const response = await getData(import.meta.env.VITE_API_URL_8086 + `customer/users?customerId=${customerId}&botId=${botId}&pageNumber=${pagination.pageNumber}&pageSize=${pagination.pageSize}`);
    return response;
  }
