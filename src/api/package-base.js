...
class PackageBasesApi {
  getPackageBases({ pageNumber, pageSize }) {
    const data = getData(
      import.meta.env.VITE_API_URL_8087 +
  ...
  getPackageBase({ packageBaseId }) {
    const data = getData(
      import.meta.env.VITE_API_URL_8087 + `package-base/detail/${packageBaseId}`
    );
    return data;
  }
  async createPackageBase(data) {
    const response = await postData(import.meta.env.VITE_API_URL_8087 + 'package-base', data);
    return response;
  }
  async updatePackageBase(data) {
    const response = await putData(import.meta.env.VITE_API_URL_8087 + 'package-base', data);
    return response;
  }
}
